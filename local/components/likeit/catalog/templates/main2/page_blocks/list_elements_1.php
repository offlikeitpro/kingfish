...
<? } ?>
<?
if (!empty($arParams["CHARTER_SECTION_ID"])) {
    $this->SetViewTarget("top_content");
    ?>
    <div class="section-block">
        <? @include_once('sections_3.php'); ?>
    </div>
    <?
    $this->EndViewTarget();
}
...
<? } ?>
<div class="right_block1 test clearfix catalog1 <?= strtolower($arTheme["FILTER_VIEW"]["VALUE"]); ?>"
     id="right_block_ajax">
    <div class="filter-panel-wrapper <? CMax::getVariable('filter_exists'); ?>">
        ...
        <? if (!$bSimpleSectionTemplate) { ?>
            ...
            <? if ($arTheme["FILTER_VIEW"]["VALUE"] == "COMPACT" || $arTheme['LEFT_BLOCK_CATALOG_SECTIONS']['VALUE'] == 'N') { ?>
                <?
                $sectionCode = explode("/", $_SERVER["SCRIPT_URL"]);
                $sectionCode = $sectionCode[count($sectionCode) - 2];
                /*$section = CIblockSection::GetList([], ["IBLOCK_ID" => 43, "CODE" => $sectionCode], false, ["ID", "UF_NO_FILTER"])->Fetch();
                if (!$section["UF_NO_FILTER"]):*/
                ?>
    ...
    <div class="inner_wrapper">
        ...
        <? if ($isAjax == "N") { ?>
            <? if ($linkedArticles && $arParams["BLOG_IBLOCK_ID"]): ?>
                ...
                ?>
                <div class="linked-blog-list <?= $linkedArticlesPos ?>" data-desktop_row="<?= $linkedArticlesRows ?>"
