...
if ($arParentSection = $rsParentSection->GetNext()) {
    ...
    $rsSect = CIBlockSection::GetList(array('left_margin' => 'asc'), $arFilter, false, $selectSection);
    while ($arSect = $rsSect->GetNext()) {
        $array[$arSect["ID"]] = $arSect["ID"];
    }
}
if (empty($array)) {
    $array = $section["ID"];
...
$arFilter = array("IBLOCK_ID" => $arParams["IBLOCK_ID"], "SECTION_ID" => $array, "ACTIVE" => "Y", '!PROPERTY_BRAND' => false);
$res = CIBlockElement::GetList(array(), $arFilter, false, false, $arSelect);
while ($ob = $res->fetch()) {
    $filterIdBrend[$ob['PROPERTY_BRAND_VALUE']] = $ob['PROPERTY_BRAND_VALUE'];
}
unset($array);
	...
	<div class="section-content-wrapper <?=($bShowLeftBlock ? 'with-leftblock' : '');?>">
		...
		<div class="js_wrapper_items" data-params='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arTransferParams, false))?>'>
			<div class="js-load-wrapper">
				...
				<?endif;?>
                <? if (!empty($filterIdBrend)) {
                    global $filterIdBrendNewsList;
                    $filterIdBrendNewsList["ID"] = $filterIdBrend;
                    ?>
                    <? include_once('page_blocks/brands_block.php'); ?>
                    <?
                    unset($filterIdBrend);
                } ?>
...
if (($_GET['PAGEN_1']) !== null) {
    $APPLICATION->setPageProperty('description', $APPLICATION->GetProperty("description") . ' - страница ' . $_GET['PAGEN_1']);
}
