...
if (!empty($arResult['ITEMS'])){
	...
	{
	    /*if($_REQUEST["dev"] == "Y"){
	        echo "<pre>", print_r($arItem["CATALOG_QUANTITY"],1), "</pre>";
	        echo "<pre>", print_r($arItem["NAME"]." avail ".$arItem["CATALOG_AVAILABLE"],1), "</pre>";
        }*/
...
/**
 * Установка на карточку товара цену торгового предложения у которого есть скидка
 */
function setPriceWithDiscount(&$arResult) {
    foreach ($arResult["ITEMS"] as $itemKey => $itemArray) {
        if ($itemArray["MIN_PRICE"]["VALUE"] == $itemArray["MIN_PRICE"]["DISCOUNT_VALUE"] && $itemArray["OFFERS"] && !empty($itemArray["OFFERS"])) {
            $originalItemMinPriceArray = $itemArray["MIN_PRICE"];
            foreach ($itemArray["OFFERS"] as $offerKey => $offerArray) {
                if ($offerArray["MIN_PRICE"]["VALUE_VAT"] != $offerArray["MIN_PRICE"]["DISCOUNT_VALUE_VAT"]) {
                    $originalItemMinPriceArray = $offerArray["MIN_PRICE"];
                    echo('<div class="devvv" style="display: none;"><pre>'); print_r($originalItemMinPriceArray); echo('</pre></div>');
                    break;
                }
            }
            $arResult["ITEMS"][$itemKey]["MIN_PRICE"] = $originalItemMinPriceArray;
        }
    }
}
setPriceWithDiscount($arResult);
// меняем ссылку на детальную карточку
foreach ($arResult['ITEMS'] as $keys => $value) {
    $newDetailPage = array_diff(explode("/", $value['DETAIL_PAGE_URL']), array(''));
    unset ($newDetailPage[2]);
    $arResult['ITEMS'][$keys]['DETAIL_PAGE_URL'] = "/".str_replace("brands", "catalog", implode("/", $newDetailPage))."/";
}
