...
<?if($arResult["ITEMS"]):?>
	...
	<?endif;?>
		...
		<?foreach($arResult["ITEMS"] as $arItem){?>
            ...
            <? ob_start(); ?>
            <?
            // переход на детльную карточку в каталоге, убираем бренд из урла
            $newDetailPage = array_diff(explode("/", $arItem['DETAIL_PAGE_URL']), array(''));
            unset ($newDetailPage[2]);
            ?>
            <div class="item-title">
                <a href="<?=$arItem['DETAIL_PAGE_URL']?>"
                   class="<?= $bBigBlock && $bFonImg ? '' : 'dark_link' ?> option-font-bold font_sm"><span><?= $elementName; ?></span></a>
            </div>
