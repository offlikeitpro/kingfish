	...
	var onLoadjqm = function(name, hash, requestData, selector, requestTitle, isButton, thButton){
		...
		if(typeof $(hash.t).data('ls') !==' undefined' && $(hash.t).data('ls'))
		{
			...
			if(typeof localStorage !== 'undefined')
			{
				...
				v={};
				v['VALUE'] = 'Y';
				v['TIMESTAMP'] = Date.now() + (ls_timeout*1000); // default: seconds for 1 day
			...
			else
			{
				var val = $.cookie(ls);
				if(!val)
					$.cookie(ls, "Y", {expires: Date.now() + (ls_timeout*1000)});// default: seconds for 1 day
			}
		}
...
$(document).ready(function(){
	...
	if(window.matchMedia('(max-width: 767px)').matches){
		...
		//console.log($('.bx_filter .scrollbar'));
	}
	/**/
	...
	$(document).on('change', "input#select_all_items", function(){
		...
		var notOfferCount = $('.table-view__item:not([data-product_type=3])').length;
		...
		if(_this.is(':checked'))
		{
			...
			//buy && wish
			$('<div class="text muted">(<span>' + notOfferCount + '</span>)</div>').appendTo($('.opt_action:not([data-action=compare])'));
	...
	if(arAsproOptions["PAGES"]["BASKET_PAGE"])
	{
		if(arAsproOptions["THEME"]["SHOW_BASKET_PRINT"]=="Y")
		{
			if($('.page-top h1').length)
				$('<div class="print-link colored_theme_hover_bg-block" title="'+arAsproOptions["THEME"]["EXPRESSION_FOR_PRINT_PAGE"]+'"><i class="svg colored_theme_hover_bg-el-svg"><svg class="svg svg-print" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"><path id="Rectangle_665_copy_4" data-name="Rectangle 665 copy 4" class="cls-print" d="M1570,210h-2v3h-8v-3h-2a2,2,0,0,1-2-2v-5a2,2,0,0,1,2-2h2v-4h8v4h2a2,2,0,0,1,2,2v5A2,2,0,0,1,1570,210Zm-8,1h4v-4h-4v4Zm4-12h-4v2h4v-2Zm4,4h-12v5h2v-3h8v3h2v-5Z" transform="translate(-1556 -197)"></path></svg></i></div>').insertBefore($('.page-top h1'));
		}
	}
	else
	{
	...
	$(document).on('click', '.js-show-info-block', function(e){
		...
		if(!$this.siblings('.js-info-block').length)
		{
			...
			$this.addClass('loadings');
			$.post(arAsproOptions['SITE_DIR']+'ajax/productStoreAmountCompact.php'+add_url, {'ajax': 'Y', 'ELEMENT_ID': $this.data('id'), 'FIELDS': dataFields, 'USER_FIELDS': dataUserFields})
				.done(function(html){
					$this.removeClass('loadings');
